...
const NavBar: React.FC<NavBarProps> = ({ showElements }) => {
  ...
  return (
    <div className="fixed top-0 z-10 flex h-24 w-full items-center justify-between px-10 backdrop-blur-lg">
      ...
      {showElements && (
        <div className="relative ml-5 md:hidden">
          ...
          {isMenuOpen && (
            <div className="absolute right-0 mt-2 flex w-52 flex-col items-center justify-around rounded-md p-3 backdrop-blur-xl">
              <Link href="/" className="mb-2 px-4 py-2 text-custom-color hover:text-white">Give Vineo as a gift</Link>
              <button type="submit" onClick={handleSignIn} className=" mb-2 w-full rounded-xl border-2 border-custom-color px-4 py-2 text-left text-custom-color hover:bg-gray-100">Access</button>
              <button type="submit" className=" rounded-full bg-green-500 px-4 py-2 text-left text-white transition-colors hover:bg-green-600">
                <AiOutlineWhatsApp />
              </button>
            </div>
          )}
